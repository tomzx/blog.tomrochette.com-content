...
- Answered in chat 2026-09-26 (owner instruction: "Let's track all model access providers and their pricing"): the [model-access](model-access/_index.md) category now exists with eleven members (OpenRouter, OpenCode Zen, OpenCode Go, Requesty, Synthetic, NanoGPT, Chutes, Cerebras Code, GLM Coding Plan, Kimi Code, MiniMax Coding Plan) and its feature matrix; that resolves the gateways question above for hosted access providers, while the self-hosted gateway candidates from it (experiential, my-free-code) remain logged rejections as gateway software, not access providers, unless you want them tracked too.
