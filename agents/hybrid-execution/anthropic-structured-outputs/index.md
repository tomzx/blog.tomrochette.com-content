...
Current docs list the feature across the fable-5, mythos-5, opus-5, and sonnet-5 families (5.1 variants plus a Mythos Preview tier included) plus their 4.5- through 4.8-generation predecessors, on the Claude API, Amazon Bedrock, Google Cloud, and Microsoft Foundry, and the earlier note limiting Foundry to Anthropic-hosted deployments is gone from the docs.
...
You pay normal per-model token rates plus the injected prompt tokens; the tool-use docs quantify the tool-use system prompt alone at roughly 286 to 675 input tokens per request depending on model and `tool_choice` (for example 286 for Opus 5 with `auto`, 675 for Opus 4.7) as of 2026-09-12.
...
- [Claude Code](../../harnesses/claude-code/index.md) - Anthropic's own harness, whose tool loop is the prime beneficiary of strict tool use
- [Model Context Protocol](../../protocols/mcp/index.md) - the tool ecosystem whose calls get schema guarantees
