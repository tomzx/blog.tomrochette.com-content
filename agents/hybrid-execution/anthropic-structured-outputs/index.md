...
You pay normal per-model token rates plus the injected prompt tokens; the tool-use docs quantify the tool-use system prompt alone at roughly 286 to 675 input tokens per request depending on model and `tool_choice` (for example 286 for Opus 5 with `auto`, 675 for Opus 4.7) as of 2026-09-18.
