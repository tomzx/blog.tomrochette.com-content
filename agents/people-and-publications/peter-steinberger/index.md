---
title: Peter Steinberger
created: 2026-09-24
updated: 2026-09-24
status: finished
tags: [research-note, agent-curated, fully-ai-generated, llm=glm-5.3-flash, people, publications, coding-agents, openclaw, developer-tools]
readability: 3
audience_notes: >
  Engineers who want the builder-evangelist view of agentic engineering, with the tooling he ships treated as the primary output.
  Assumes you know what OpenClaw, a coding agent subscription, and a menu bar app are.
---

Peter Steinberger is the PSPDFKit founder who came out of retirement to create OpenClaw, joined OpenAI in February 2026, and publishes hands-on agent-engineering essays at steipete.me alongside a torrent of open-source agent tooling on GitHub.
Facts below verified as of 2026-09-24.

**He is the builder-evangelist of the agent wave: his value is working tools and first-person workflow writing at enormous volume, and his boundary is that he is always selling the future he is personally building.**

## What it is

A personal blog with practitioner essays ("Just Talk To It", "Claude Code Anonymous", "Shipping at Inference-Speed") plus a GitHub fleet of dozens of single-purpose agent tools: CodexBar (21.8k stars as of 2026-09-24), Peekaboo, mcporter, oracle, agent-scripts, and many more, 67k+ stars across personal projects by his own count.
He created OpenClaw, which the section's OpenClaw note records at about 390k stars with near-daily releases as of 2026-09-22, bootstrapped PSPDFKit to a nine-figure exit, burned out, retired, and then says AI pulled him back in.
Since February 2026 he has worked at OpenAI on bringing agents to everyone, while OpenClaw moved to an independent foundation he stewards.
The audience is builders: his Claude Code Anonymous meetups spread to at least six cities on the rule "show us what you learned, not what you sell".

## Status

Active, but the venue shifted in 2026.
The blog published more than thirty posts between June and December 2025, then one in 2026, the 15 February OpenAI announcement, which drew a 1,449-point Hacker News thread with 1,131 comments as of 2026-09-24.
He had 52.9k GitHub followers as of 2026-09-24, a TED 2026 talk on OpenClaw, and a Pragmatic Engineer feature in his recognition list.
OpenClaw keeps shipping without his daily hand, and the section's note tracks its releases and its 2026 provider-restriction saga.

## Strengths

- Tool density nobody matches: when he hits a friction he ships the fix, and the ecosystem ports it (a CodexBar for Android appeared within weeks).
- The workflow essays set culture: "Claude Code Anonymous" named the obsession, the sleep loss, and the full-breadth-developer adoption pattern, then turned it into a meetup format.
- OpenClaw is the field's largest proof that a personal agent can be owned rather than rented, and his handoff announcement is the primary source for the foundation structure.
- A long public track record (iOS and Swift prominence, PSPDFKit) means his practitioner claims come with two decades of receipts.

## Cautions

- **Since February 2026 his sustained commentary effectively lives on X, which this run could not fetch, so his current positions are hard to cite and easy to misquote.**
- The evangelist stack is thick: an OpenAI salary, foundation stewardship, and an ecosystem around his own tools, so treat his productivity claims as marketing-adjacent.
- The self-narrative runs hot (16-hour days, 20x-faster building, slot-machine metaphors), and the pushback is part of the record: a managed-hosting founder built his product specifically against Steinberger's "if you can't use a terminal you shouldn't use OpenClaw" stance, and a security-tool author criticized credential advice in a ClawHub skill published under his name.

## Compared to

- [Steve Yegge](../steve-yegge/index.md): both are loud advocates; Yegge argues in essays, Steinberger argues by shipping, so choose Steinberger for artifacts and Yegge for rhetoric.
- [Simon Willison](../simon-willison/index.md): Willison documents what tools do, Steinberger embodies what tools feel like; Willison for evidence, Steinberger for energy and tooling.
- [Armin Ronacher](../armin-ronacher/index.md): the skeptic to his evangelist; reading both gives you the whole argument.

## Bottom line

**Recommended for engineers who want working agent tooling and the practitioner culture growing around it.**
Not for neutral analysis or anyone allergic to hype; his incentives follow his projects.

## Top 5 recommended reading

- [Just Talk To It - the no-bs Way of Agentic Engineering](https://steipete.me/posts/just-talk-to-it.md) - His philosophy essay for agentic engineering, the cleanest introduction to how he actually works with agents.
- [Claude Code Anonymous](https://steipete.me/posts/2025/claude-code-anonymous.md) - The piece that named the practitioner obsession with coding agents and turned it into a meetup format now in six cities.
- [OpenClaw, OpenAI and the future](https://steipete.me/posts/2026/openclaw.md) - His announcement joining OpenAI and handing OpenClaw to a foundation, the primary source for both facts.
- [Shipping at Inference-Speed](https://steipete.me/posts/2025/shipping-at-inference-speed.md) - His account of building at model-speed, the clearest statement of his working method.
- [CodexBar](https://github.com/steipete/codexbar) - His most-starred solo tool, usage stats for Codex and Claude Code in the menu bar, the working example of his ship-the-fix habit.

## Changes

- 2026-09-24 - Created.
- 2026-09-24 - Added the Top 5 recommended reading section.

## See also

- [OpenClaw](../../assistant-runtimes/openclaw/index.md) - the project he created and handed to a foundation
- [Simon Willison](../simon-willison/index.md) - the chronicler to his evangelist
- [Steve Yegge](../steve-yegge/index.md) - the other big-voice advocate, in essay form
- [Mario Zechner](../mario-zechner/index.md) - the small-core counterpoint to his platform-scale bet
- [Pi](../../harnesses/pi/index.md) - the harness the OpenClaw ecosystem builds on

## References

- https://steipete.me/ - site home and identity line
- https://steipete.me/about.md - OpenClaw creator, OpenAI since February 2026, PSPDFKit exit, San Francisco
- https://steipete.me/posts.md - the full post archive grounding the 2025 density and the 2026 drop
- https://steipete.me/posts/2026/openclaw.md - the OpenAI announcement and the OpenClaw foundation handoff
- https://steipete.me/posts/2025/claude-code-anonymous.md - the meetup format and its culture diagnosis
- https://github.com/steipete - profile: 52.9k followers, the tool fleet, star counts, TED talk, Pragmatic Engineer feature
- https://hn.algolia.com/api/v1/search?query=steipete&tags=story&hitsPerPage=10 - the 1,449-point announcement thread, the CodexBar ports, and the critical replies
