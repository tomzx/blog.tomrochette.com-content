...
| Maturity | v1.0.1 (2026-05) | version 1, v2 draft, remote WIP | packages 0.0.59, spec 1.0 draft | v0.9.0 (2026-08-28) | unversioned, de facto standard | dated revisions (2026-07-28) |
...
MCP is table stakes across the harness and surface matrices; AGENTS.md counts more than 60,000 carrying projects with one glaring holdout (Claude Code); ACP rides OpenCode, JetBrains, Zed, and a Copilot CLI preview; A2A has no native speaker among this index's harnesses, only community setups near Gemini CLI; AHP is just over six months old with 338 stars as of 2026-09-16, but its reference host ships inside VS Code and the VS Code team has said publicly it is rebuilding its agent infrastructure on the protocol.
The SDK download ratio recorded in the A2A note, 10.9M monthly versus 257M for MCP, is the gap in one number, and AHP's roughly 302k combined downloads (about 225k crates plus 77k npm) show the same order-of-magnitude distance from the top.
AG-UI is the anomaly that proves the rule: none of this index's harnesses or surfaces speak it natively, yet its SDKs moved about 12M combined npm downloads in the last month as of 2026-09-16, because the frontend layer is where end-user products live even though coding tools never touch it.
...
- Streaming an agent into a product frontend: AG-UI, accepting CopilotKit's stewardship and 0.0.x package churn.
...
- https://github.com/ag-ui-protocol/ag-ui - repository facts (15,915 stars, MIT, created 2025-05-07, pushed 2026-09-15) for the AG-UI column (GitHub API, as of 2026-09-16)
...
- https://docs.ag-ui.com - TypeScript, Python, and .NET SDKs and the draft 1.0 specification for the AG-UI column
- https://api.npmjs.org/downloads/point/last-month/@ag-ui/core - 7,171,054 downloads for the adoption paragraph
- https://api.npmjs.org/downloads/point/last-month/@ag-ui/client - 4,855,689 downloads for the adoption paragraph
