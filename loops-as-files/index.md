...
That covers the common case of "I want this skill to fire on a cron *or* when a human pokes it," which is exactly the pattern of most operational loops.
...
## The Other Axis Is Authority, Not Just Cost

The budget field answers how much a loop may spend.
It does not answer the question that matters more once a loop runs while you sleep: what is this loop allowed to do, even if it costs nothing?
Filing an issue is cheap.
Pushing to `main`, posting to a customer channel, or opening a PR that auto-merges is cheap too, and at each of those "cheap" is the wrong axis.

So the format needs a second bound alongside `on` and `budget`, and the natural name for it is authority.
I am confident the axis exists and that the frontmatter has to carry it.
I am not confident the field is easy to design, because the leverage to enforce it is not obvious.

The naive sketch borrows from GitHub Actions and from every capability system before it:
...
autonomy: propose   # ask | propose | full
```

That sketch is the easy part.
The hard part, which it hides, is that the harness does the heavy lifting.
The runtime does not load the skill or call the model or route tool calls, the harness does.
So the real question is where the runtime gets the authority to refuse a side effect the model decided to make, and the answer is that it has that authority only where it controls what the harness is given or sits on the only wire out.

The levers exist, but each depends on something.
Credential scoping needs no cooperation: the runtime mints the token, a `write: [issues]` token physically cannot push, the upstream API enforces it.
Tool mounting needs a harness that exposes a configurable tool surface.
Tool-call interception needs the runtime to proxy every tool, the way a service mesh sits on the wire, which is real engineering.
And a closed harness that demands full credentials and full filesystem collapses the whole thing to a container sandbox, at which point `ask` and `propose` degrade to "run it or don't."

That is why I am flagging authority as open rather than shipping it as solved.
The `autonomy` ladder is worth wanting: most loops should start at `propose`, where every write is intercepted and staged into a draft PR or a prepared message with one approve button, and graduate to `full` only after enough clean runs that the promotion is boring.
Skipping that graduation is where the incidents live.
But `propose` works only if the runtime can intercept every write, which is the one lever that depends most on the harness, so the ladder describes the goal while the hard work is making interception real on the harnesses people actually run.

Authority is the budget field's partner: budget bounds how much, authority bounds what.
Both belong in the frontmatter.
I am not going to pretend the mechanism that delivers authority is as settled as the one that delivers budget.

## State Is the Loop's Memory

A loop that says "for every issue opened since the last run" carries a hidden dependency: the runtime must remember what the last run saw.
That memory is not an implementation detail.
It is part of the contract, because without it the loop is either redoing work or skipping it, and nothing in the file tells you which.

This is the part of the format I am least confident about, because declaring a cursor is easy and advancing it correctly is hard.
The naive declaration is just a key in the frontmatter:

```yaml
state:
  key: last-seen-issue-number
```

That is the easy half.
The hard half is who computes the next value, and from what.

For the common case it is almost free.
When the trigger is an event that carries its own ordering, the issue number inside the `github.issues.opened` payload, the commit sha inside the push, the timestamp inside the message, the runtime can advance the cursor itself as the high-water mark of the events it dispatched.
The skill never touches state, which is the clean outcome the how/when split wants.

The moment the meaningful watermark is not in the event, the contract stops being automatic.
"Last row processed in a database," "last log line consumed," "last file checksummed" are cursors the event does not carry and the runtime cannot invent.
Something has to compute them from the run's outputs, and that something is either the skill, which breaks the separation because now the skill knows about the loop's memory, or the loop body, declaring an assignment the runtime persists.
Neither is obviously right, and I am not going to pretend the format has settled it.
Every cursor not derivable from the event is a place where the contract leaks into the skill, and a loop format that claims to support state has to own that leak rather than hide it behind a field.

The same problem recurs at finer grain as the dedup key.
A fingerprint derived from the event lets the runtime answer "have I already handled this exact trigger" before it spends a token, and for event-driven loops that fingerprint is free for the same reason the cursor is.
For everything else, idempotency is a claim the skill makes and the runtime trusts, which is the failure mode worth catching in review.

The structure this points toward is the per-run state file from [Managing Many Concurrent LLM Agent Sessions](../managing-many-llm-agent-sessions/index.md), lifted from the session layer to the loop layer.
But lifting the structure does not answer the cursor-advancement question, and I would rather flag it as open than ship a field that implies it is solved.

## Testing the Trigger, Not Just the Skill

A skill is testable because it is a function: inputs in, assertion out.
A trigger has historically been untestable, because it lived in a crontab or a webhook config with no inputs you could hand it.
A loop file fixes that, because the trigger is now data, and data can be replayed.

Capture a real event (a webhook payload, an issue body, a metric spike) as a fixture.
Point `--dry-run` at the fixture and assert the loop would have fired, or would have been filtered out.
The filter `author.trust < 'trusted'` is no longer a guess, it is a line of coverage, and a regression in the trust model surfaces as a failing test before it surfaces as a misrouted issue.

This is the property that makes loops safe to refactor.
Without replay tests, changing a filter is an act of faith with a 03:00 page at the end.
With them, the trigger layer gets the same feedback loop the skill layer already has, and the two halves of the behavior finally evolve under the same discipline.

## Loops That Maintain Loops

The recursive move is the one that changes what the system is.
Once loops are files, a loop can read, write, and retire other loops, and the moment that is true the scheduling layer starts to maintain itself.

A weekly health loop is enough to begin.

```markdown
# Weekly Loop Health

1. For each loop in `loops/`, dry-run its trigger against the last week of events.
2. For any loop that has not succeeded in 30 days, `/create-pr` to archive it.
3. For any loop whose cost-per-useful-outcome is above threshold, `/create-pr` to tighten its budget.
4. Summarize the rest in a comment to each loop's owner.
```

This is the [Self-Evolving Repository](../the-self-evolving-repository/index.md) made literal.
The self-evolution was always going to stay hand-wavy until there was a file format the meta-process could operate on, and the loop file is that format.
A repository that schedules its own skills, and prunes its own schedules, is no longer a metaphor once the schedules are markdown in git.

The guardrail is the same one this whole piece argues for: a meta-loop is still a loop, so it gets a budget, an authority tier, and an owner, and the first one ships at `autonomy: propose` for the obvious reason.
Its one specific failure mode is that a loop-retiring loop can retire itself.
Give every meta-loop `protected: true`, so the system keeps a floor it cannot pull out from under itself.
