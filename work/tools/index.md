---
title: Tools
created: 2024-08-26
type: post
status: draft
tags: [work]
readability: 1
---

Tools I use on a regular basis at work (sorted alphabetically).

* [Apple calendar](https://www.apple.com/macos/calendar/)
* [Chrome](https://www.google.com/chrome/)
* [`claude` (Claude Code)](https://docs.anthropic.com/en/docs/claude-code)
* [Cursor](https://www.cursor.com/)
* [ `cursor-agent` (Cursor agent CLI)](https://cursor.com/cli)
* [`gh` (GitHub CLI)](https://cli.github.com/)
* [`gh-cached`](https://github.com/TomzxCode/gh-cached)
* [`ghx`](https://github.com/TomzxCode/ghx)
* [`git`](https://git-scm.com/)
* [GitHub](https://github.com/)
* [`gt` (Graphite CLI)](https://graphite.dev/)
* [`gcloud` (Google Cloud CLI)](https://cloud.google.com/sdk/gcloud)
* [Google suite](https://workspace.google.com/) (Calendar, Docs, Meet, Sheets, Slides)
* LLMs ([ChatGPT](https://chatgpt.com/), [Claude](https://claude.ai/))
* [iTerm2](https://iterm2.com/)
* [Jetbrains Toolbox](https://www.jetbrains.com/toolbox-app/)
* [OpenChamber](https://openchamber.dev/)
* [`opencode` (OpenCode)](https://opencode.ai/)
* [OpenLens](https://github.com/MuhammedKalkan/OpenLens)
* [`podman` (Podman)](https://podman.io/)
* [Postman](https://www.postman.com/)
* [PyCharm](https://www.jetbrains.com/pycharm/)
* [Rectangle](https://rectangleapp.com/)
* [Slack](https://slack.com/)
* [Stats](https://github.com/exelban/stats)
* [VS Code](https://code.visualstudio.com/)
