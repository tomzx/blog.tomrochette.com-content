...
type: post
status: finished
tags: [artificial-general-intelligence, fully-ai-generated, llm=gemini-3-fast, llm=glm-5.3]
readability: 5
audience_notes: >
  Written for a general technical audience: no machine learning background is assumed, LLMs are introduced from scratch, and the biology parallels are explained as they appear.
---
...
In the traditional world of coding, software is "built."
An architect designs a blueprint, and developers lay the bricks.
**But in the era of Generative AI, we are moving toward a paradigm where software is evolved.**

Darwinian Software Development is an emerging methodology that uses "swarms" of Large Language Models (LLMs) to iterate, compete, and select the most efficient code through a process mirroring [natural selection](https://en.wikipedia.org/wiki/Natural_selection).
...
The process generally follows four biological phases:
...
This failure mode is known as [reward hacking](https://en.wikipedia.org/wiki/Reward_hacking).
To counter reward hacking, modern swarms often include "Adversarial Agents" whose only job is to write difficult unit tests to try and "kill" the code generated by the other agents.
