...
For most of software's history, filing an issue was a small act of labor.
You had to reproduce the bug, narrow it down, write a coherent description, and decide whether the request was worth the maintainer's time.
That filter is gone.
**An AI agent can read a README, a changelog, or a ten-minute session with the tool, and produce a polished, well-formatted issue in seconds, and it can do it a hundred times before lunch.**
The cost of creating an issue collapsed, the cost of clearing one did not, and every issue tracker on the internet is now a faucet pointed at a thimble.

If you are the one filing those issues, the implication is uncomfortable but precise.
The issue you just opened is no longer a contribution in any meaningful sense.
It is a request for someone else's labor, and it is sitting in a queue next to nine hundred other requests for someone else's labor, most of them produced just as cheaply as yours.
**The only thing that still moves the needle is the implementation attached to the request, because the implementation is the one input whose cost has not collapsed.**

## The Issue Tracker Was Always a Wish List

It is worth being precise about what an issue tracker actually was, even before AI.
...
## The Cost Asymmetry Is the Whole Problem
...
**One side of that transaction is free; the other side is unpaid, and the unpaid side is the one you are asking to work.**

Multiply that asymmetry by a few hundred issues a week and you get the current state of every popular open source repository on the planet.
The triage queue grows faster than any single human can read it, the maintainer starts ignoring the tab, the contributor waits six months for a reply, and the project slowly acquires a reputation for being unresponsive.
Nobody is at fault.
...
Here is the move that fixes the asymmetry, and it is the only move that fixes it.
...
Evaluating the work is bounded, fast, and something a maintainer can actually fit into a Saturday morning.
The difference between "you should add a dark mode" and "here is a dark mode, here are the tests, here is the screenshot, merge or reject" is the difference between a request for charity and an offer to collaborate.
...
A PR gives them a binary decision they can make in minutes.
...
## The Cost of Implementing Collapsed Too

The obvious objection is that not everyone can write the implementation, and that used to be true.
...
This is the same relocation [The Shifting Bottleneck](../the-shifting-bottleneck/index.md) keeps arriving at.
...
This is the second objection, and it is more substantive than the first.
...
The answer is not to refrain from sending the PR.
...
A sprawling, untested, AI-generated diff with a one-line description is just a different kind of issue, and it will be treated like one.
...
They do not want to spend it transcription-editing a feature description into code, because that is the part the LLM can do now.
...
There is a real exception, and it should be named so the argument does not overreach.
...
**File the issue only if the PR attempt genuinely failed, and when you do, include what you learned from the attempt, because that is the information the maintainer actually needs.**
...
Be explicit that this is not hostility.
...
And for everyone, internalize the new cost structure.
Ideas are free.
...
It is that implementations are the one input whose cost has not collapsed to zero, which makes them the one input that still signals you meant it.
