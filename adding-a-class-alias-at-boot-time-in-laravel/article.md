...
When I work with Laravel, I make extensive use of [Laravel](https://www.exolnet.com/en/blog/what-is-laravel-modern-php-framework)  [Debugbar](https://github.com/barryvdh/laravel-debugbar) to track performance of parts of my application. I sprinkle calls to `Debugbar::startMeasure` and `Debugbar::stopMeasure` to track the duration of certain segments of my code. However, when this code goes into production, this dependency isn't present. This causes the code to break since it cannot find `Debugbar` anymore.
