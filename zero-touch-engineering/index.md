...
Zero touch engineering has a clear lineage in networking and operations, and that lineage explains why software engineering was the last holdout and what it takes to close the gap.
...
You trust the verification system.
...
Feature requests as the only signal produce a system that accumulates features and loses coherence.
That drift is [Goodhart's law](https://en.wikipedia.org/wiki/Goodhart%27s_law) applied to engineering: when a signal becomes the target of an autonomous system, it stops being a good signal.
...
The pragmatic answer is not to solve the direction problem perfectly but to bound it.
...
That split keeps the loop touchless for the vast majority of decisions while preserving human oversight for the small fraction that set long-term trajectory.
